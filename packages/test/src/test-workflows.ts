...
async function runPatchedWorkflowWithCallback(
  ...
  if (threaded) {
    ...
    if (REUSE_V8_CONTEXT) {
      creator = await TestReusableVMWorkflowCreator.create(t.context.workflowBundle, 400, new Set(), internalCallback);
    } else {
      creator = await TestVMWorkflowCreator.create(t.context.workflowBundle, 400, new Set(), internalCallback);
    }
  }
